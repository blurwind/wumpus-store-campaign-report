...
async function dailyEmbed(webhookUrl, botId) {
  const wumpusStatsRequest = await axios.get(
    `https://stats.wumpus.store/api/stats/wumpus.store/top-stats?period=day&date=${today}&filters=%7B%22page%22%3A%22%2Fbot%2F${botId}%22%7D&with_imported=true&auth=oQLzDx_H5lHWjQ6YkEFhe&comparison=previous_period&compare_from=undefined&compare_to=undefined&match_day_of_week=true`
  );
  ...
  const embed = {
    ...
    embeds: [
      {
        title: `Daily Campaign Report for ${botInfoRequest.data.data.username}`,
        url: `https://wumpus.store/bot/${botId}`,
        description: `📊 Here is your daily campaign report for ${botInfoRequest.data.data.username} on [Wumpus.store](https://wumpus.store/bot/${botId})!`,
